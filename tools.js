...
export default class Tools {
  ...
  static ordernarAZ( lista, nomeCampo) {
  ...
  static mascara_telefone(v) {
    if (this.isEmpty(v)) return v
  ...
  static filterlist(lista = [], text = String, filtros = []) {
    if (this.isEmpty(text.trim())) return lista;
    let lista_filtrada = []
    lista.map(item => filtros.map(f => {
      if (lista_filtrada.indexOf(item) > -1) return null
      if (f.length > 1) {
        let arvore = item
        f.map(i => arvore = arvore[i])
        return String(this.retira_acentos(String(arvore).toUpperCase())).indexOf(String(text).toUpperCase()) > -1 ? lista_filtrada.push(item) : null
      }
      else {
        return this.retira_acentos(String(item[f]).toUpperCase()).indexOf(String(text).toUpperCase()) > -1 ? lista_filtrada.push(item) : null
      }
    }));
  ...
  static modalidade() {
    let data = [
      {
        "value": "0",
        "label": "Por conta do emitente (CIF)"
      },
      {
        "value": "1",
        "label": "Por conta do destinatário/remetente (FOB)"
      },
      {
        "value": "2",
      ...
      {
        "value": "9",
        "label": "Sem Ocorrência de Transporte (Sem frete)"
      }
    ]
    return data
  }
  ...
  static tipo_conta() {
    let data = [
      {
        "value": "001",
        "label": "Conta Corrente de Pessoa Física"
      },
      {
        "value": "002",
        "label": "Conta Simples de Pessoa Física"
      },
      {
        "value": "003",
        "label": "Conta Corrente de Pessoa Jurídica"
      },
      {
        "value": "006",
      ...
      {
        "value": "007",
        "label": "Depósitos Instituições Financeiras"
      },
      {
        "value": "013",
        "label": "Poupança de Pessoa Física"
      },
      {
        "value": "022",
        "label": "Poupança de Pessoa Jurídica"
      },
      {
        "value": "028",
        "label": "Poupança de Crédito Imobiliário."
      },
      {
        "value": "043",
        "label": "Depósitos Lotéricos."
      },
    ]
    return data
  }
  ...
  static downloadFileString(stringFile, formato) {
    ...
    var filename = "file." + formato ;
    var pom = document.createElement('a');
    var bb = new Blob([stringFile], { type: 'text/plain' });
